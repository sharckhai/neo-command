...
def search_facilities_multi(
    ...
    *,
    ...
    for nid, ndata in G.nodes(data=True):
        ...
        # Confidence: average confidence of matched edges
        confidence = 0.0
        conf_count = 0
        for _, target, edata in G.edges(nid, data=True):
            etype = edata.get("edge_type")
            if etype in (EDGE_HAS_CAPABILITY, EDGE_HAS_EQUIPMENT, EDGE_HAS_SPECIALTY):
                confidence += edata.get("confidence", 0)
                conf_count += 1
        confidence = round(confidence / conf_count, 3) if conf_count else 0.5
    ...
    *,
    ...
    for nid, ndata in G.nodes(data=True):
        ...
        if group_by == "region":
            ...
            for _, target, edata in G.edges(nid, data=True):
                if edata.get("edge_type") == EDGE_HAS_SPECIALTY and edata.get("confidence", 0) >= min_confidence:
                    skey = _extract_key(target)
                    counts[skey] = counts.get(skey, 0) + 1

        elif group_by == "capability":
            for _, target, edata in G.edges(nid, data=True):
                if edata.get("edge_type") == EDGE_HAS_CAPABILITY and edata.get("confidence", 0) >= min_confidence:
                    ckey = _extract_key(target)
                    counts[ckey] = counts.get(ckey, 0) + 1

        elif group_by == "equipment":
            for _, target, edata in G.edges(nid, data=True):
                if edata.get("edge_type") == EDGE_HAS_EQUIPMENT and edata.get("confidence", 0) >= min_confidence:
